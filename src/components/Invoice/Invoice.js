...
const Invoice = () => {
  ...
  const handleDelivery =(e) => {
    ...
    setInvoiceData((prevState) => ({...prevState, delivery: e.target.value}))
  }
    ...
    useEffect(() => {
        const total =() => {
            ...
            const overallSum = (isNaN(parseFloat(subTotal)) ? 0 : parseFloat(subTotal)) +
            (isNaN(parseFloat(delivery)) ? 0 : parseFloat(delivery)) -
            (isNaN(parseFloat(rates)) ? 0 : parseFloat(rates));
        ...
        total()
    }, [invoiceData, rates, subTotal, delivery])
    

    const handleAddField = (e) => {
        e.preventDefault()
        setInvoiceData((prevState) => ({...prevState, items: [...prevState.items,  {itemName: '', date: '', unitPrice: '', quantity: '', discount: '', amount: '' }]}))
    }
    ...
    <div>
        ...
        <Table className={classes.table} aria-label="simple table">
            ...
            {invoiceData.items.map((itemField, index) => (
                <TableRow key={index}>
                <TableCell  scope="row" style={{width: '30%' }}> <InputBase style={{width: '100%'}} outline="none" sx={{ ml: 1, flex: 1 }} type="text" name="itemName" onChange={e => handleChange(index, e)} value={itemField.itemName} placeholder="Item name or description" /> </TableCell>
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="date" name="date" onChange={e => handleChange(index, e)} value={itemField.date} placeholder="0" /> </TableCell>
                ...
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="number" name="amount" onChange={e => handleChange(index, e)}  value={(itemField.quantity * itemField.unitPrice) } disabled /> </TableCell>
